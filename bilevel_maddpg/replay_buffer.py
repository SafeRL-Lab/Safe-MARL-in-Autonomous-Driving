...
class Buffer:
    def __init__(self, args):
        ...
        for i in range(self.args.n_agents):
            ...
            self.buffer['u_next_%d' % i] = np.empty([self.size, self.args.action_shape[i]])
            self.buffer['t_%d' % i] = np.empty([self.size, self.args.terminal_shape[i]])
        # thread lock
    ...
    def store_episode(self, o, u, r, o_next, u_next, t, c=0):
        ...
        for i in range(self.args.n_agents):
            with self.lock:
                ...
                self.buffer['u_next_%d' % i][idxs] = u_next[i]
                self.buffer['t_%d' % i][idxs] = t[i]
